...
# Org-specific skills exempt from sanitization
SANITIZATION_ALLOWLIST=("pulse-context" "design-system" "prospect-digest" "create-asana-engagement-project" "productive-api-patterns")
...
" 2>/dev/null) || { fail "$INDEX_FILE" "index-parse" "Failed to parse skill-index.json." "Check JSON syntax."; echo ""; return; }
  ...
  # Check for index entries with no disk directory
  for key in "${index_keys[@]}"; do
    local found=false
    for skill in "${disk_skills[@]}"; do
      local lower
...
content = open("$skill_file").read()
issues = []
if "/Users/snavarro/" in content:
    issues.append("hardcoded path /Users/snavarro/")
emails = re.findall(r"[\w.+-]+@servant\.io", content)
if emails:
    issues.append("email(s): " + ", ".join(set(emails)))
slack_ids = re.findall(r"U(?=[A-Z0-9]*[0-9])[A-Z0-9]{8,}", content)
if slack_ids:
    issues.append("Slack user ID(s): " + ", ".join(set(slack_ids)))
for issue in issues:
    print(issue)
PYEOF
    ) || continue
...
import json
import os
index_path = os.path.expanduser("~/.claude/skills/skill-index.json")
...
# ─── Check 8: Paired-Example Validation ──────────────────────────────

check_paired_examples() {
  local skill_dirs=("$@")
  local checked=0

  echo "=== Paired-Example Validation ==="
  echo ""
  ...
  for skill in "${skill_dirs[@]}"; do
    ...
    # Skip allowlisted (org-specific) skills
    in_allowlist "$skill" && continue

    local findings
    findings=$(SKILL_FILE="$skill_file" python3 << 'PYEOF'
import re, sys, os
...
for sec in sections:
    header = sec.split('\n')[0].strip()
    if header.lower().startswith('workflow routing'):
        routing_text = sec
    elif header.lower() == 'examples':
        examples_text = sec

has_routing = bool(routing_text.strip())
has_examples = bool(examples_text.strip())

if not has_routing and not has_examples:
    sys.exit(0)

if has_routing and not has_examples:
    print("WARN|paired-no-examples|Has Workflow Routing but no Examples section — workflows are undocumented|Add an ## Examples section with at least 2 examples demonstrating workflows.")
    sys.exit(0)

if has_examples and not has_routing:
    print("WARN|paired-no-routing|Has Examples but no Workflow Routing table — examples can't be paired|Add a ## Workflow Routing table listing workflows and triggers.")
    sys.exit(0)

routing_workflows = set(re.findall(r'\*\*(\w+)\*\*', routing_text))
example_refs = re.findall(r'Invokes\s+(\w+)\s+workflow', examples_text)
example_ref_set = set(example_refs)
example_count = len(re.findall(r'\*\*Example\s+\d+:', examples_text))

for ref in sorted(example_ref_set):
    if ref not in routing_workflows:
        print(f"FAIL|paired-unknown-workflow|Example references workflow '{ref}' but it's not in Workflow Routing table|Add '{ref}' to the Workflow Routing table or fix the example reference.")

for wf in sorted(routing_workflows):
    if wf not in example_ref_set:
        print(f"WARN|paired-uncovered-workflow|Workflow '{wf}' has no example demonstrating its usage|Add an example that invokes the {wf} workflow.")

if example_count < 2:
    print(f"WARN|paired-min-examples|Has {example_count} example(s), canonical minimum is 2|Add examples to reach at least 2 total.")

# Signal that validation ran (distinguishes "checked clean" from "skipped")
print("OK")
PYEOF
    ) || continue

    # Empty output = neither section present → skip silently
    [[ -z "$findings" ]] && continue

    while IFS='|' read -r severity rule message fix; do
      [[ -z "$severity" ]] && continue
      [[ "$severity" == "OK" ]] && continue
      case "$severity" in
        FAIL) fail "$skill/SKILL.md" "$rule" "$message" "$fix" ;;
        WARN) warn "$skill/SKILL.md" "$rule" "$message" "$fix" ;;
      esac
    done <<< "$findings"
  ...
  [[ $checked -gt 0 ]] && pass "paired-examples" "$checked"
  echo ""
}
...
main() {
  ...
  check_paired_examples "${skill_dirs[@]}"
