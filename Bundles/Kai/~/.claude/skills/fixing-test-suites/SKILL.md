...
_auth_patch = patch("api.middleware.auth.CLERK_SECRET_KEY", None)
_auth_patch.start()

# NOW import - decorator sees CLERK_SECRET_KEY as None and skips auth
...
def bypass_auth():
    ...
    with patch("api.middleware.auth.CLERK_SECRET_KEY", None):
        yield
...
env:
  ...
  CLERK_SECRET_KEY: ""  # Empty = auth bypassed
```
